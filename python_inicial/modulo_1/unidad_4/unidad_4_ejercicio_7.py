...
def recorrer_lista(item = lista, nivel_anidamiento_local = 0):
    for x in item:
        aux = isinstance(x, list)
        if aux:
            nivel_anidamiento_local += 1
            recorrer_lista(x, nivel_anidamiento_local)
            
        else:
            str_aux = nivel_anidamiento_local*"  " + x
            print(f"{str_aux}, {nivel_anidamiento_local}")
